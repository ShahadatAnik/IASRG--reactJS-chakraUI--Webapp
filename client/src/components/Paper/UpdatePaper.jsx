...
export default function UpdatePaper() {
  ...
  const firstUpdate = fp => {
    setPaper(prevState => ({
      ...prevState,
      name: fp[0].paper_name,
      publication_date: fp[0].publication_date.split('T')[0],
      publication_place: fp[0].publication_place,
      full_text: fp[0].full_text,
    }));
  };
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:3001/paper/getPaper/')
      .then(resp => resp.json())
      .then(resp => firstUpdate(resp.filter(paper => paper.id == paperID)))
  ...
  const onSubmit = e => {
    if (isInvalidName || isInvalidDate || isInvalidPlace || isInvalidFullText) {
      ...
      Axios.post('http://localhost:3001/paper/edit_paper/', {
        paper_id: paperID,
